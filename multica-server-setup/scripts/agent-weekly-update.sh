...
#   1. dsh CLI（npm 全局包，更新到 latest 后强制验证 --probe，失败自动回退 rc.6）
...
# crontab 环境 PATH 精简：nvm bin 或 ~/.npm-global/bin 在最前（保证 npm 是用户可写的），
# 系统目录兜底放最后
...
# ---------- 2. dsh ----------
DSH_VER_REQUIRED="0.1.0-rc.6"   # 桥插件验证组合；更新后 --probe 失败时回退到此版本
if command -v dsh >/dev/null 2>&1; then
    log "==> dsh update (当前 $(dsh --version 2>/dev/null))"
    ...
    if dsh --profile multica --probe >/dev/null 2>&1; then
        log "==> dsh $(dsh --version) --probe 验证通过"
    else
        log "!! dsh 更新后 --probe 失败（可能版本错位），自动回退到 $DSH_VER_REQUIRED"
        timeout 600 npm install -g "@deepseek-ai/dsh@$DSH_VER_REQUIRED" \
            && dsh --profile multica --probe >/dev/null 2>&1 \
            && log "==> 已回退 $DSH_VER_REQUIRED 且 --probe 通过" \
